...
| EC-N4 | `name` bị `null`                              | Không gửi trường name | Thăm dò             | Giả định — CẦN XÁC NHẬN        |
...
| EC-P8 | Phone bị `null` hoặc không gửi                       | Không gửi phone              | Thăm dò                       | Giả định — CẦN XÁC NHẬN |
...
| TC-B2 | EC-N2 (Name rỗng)         | Body: `{ name: "", phone: "0912345678" }`             | `400` — name bắt buộc        | `200 OK`{ "message": "Profile updated" }     | Failure |
| TC-B3 | EC-N3 (Name chỉ có space) | Body: `{ name: " ", phone: "0912345678" }`            | `400` — name không hợp lệ    | `200 OK`{ "message": "Profile updated" }     | Failure |
| TC-B4 | EC-N4 (Name null)         | Body: `{ name: null, phone: "0912345678" }`           | `400` — name bắt buộc        | `200 OK`{ "message": "Profile updated" }     | Failure |
| TC-B5 | EC-N6 (Name > 255 ký tự)  | Body: `{ name: "A"*256, phone: "0912345678" }`        | `400` — name vượt max length | `200 OK`{ "message": "Profile updated" }     | Failure |
...
| TC-C3  | EC-P3 (Phone 9 số)                   | Body: `{ name: "Test User", phone: "091234567" }`     | `400` — phone phải 10–11 số       | `200 OK`{ "message": "Profile updated" } | Failure                                                                                     |
| TC-C4  | EC-P4 (Phone 12+ số)                 | Body: `{ name: "Test User", phone: "0912345678901" }` | `400` — phone phải 10–11 số       | `200 OK`{ "message": "Profile updated" } | Failure                                                                                     |
| TC-C5  | EC-P5 (Phone không bắt đầu bằng 0)   | Body: `{ name: "Test User", phone: "9123456789" }`    | `400` — phone phải bắt đầu bằng 0 | `200 OK`{ "message": "Profile updated" } | Failure                                                                                     |
| TC-C6  | EC-P6 (Phone có ký tự không phải số) | Body: `{ name: "Test User", phone: "0912345abc" }`    | `400` — phone phải là số          | `200 OK`{ "message": "Profile updated" } | Failure                                                                                     |
| TC-C7  | EC-P7 (Phone rỗng)                   | Body: `{ name: "Test User", phone: "" }`              | `400` — phone không hợp lệ        | `200 OK`{ "message": "Profile updated" } | Failure                                                                                     |
| TC-C8  | EC-P8 (Phone null)                   | Body: `{ name: "Test User", phone: null }`            | `400` — phone không hợp lệ        | `200 OK`{ "message": "Profile updated" } | Failure                                                                                     |
| TC-C9  | EC-P8 (Không gửi phone)              | Body: `{ name: "Test User" }`                         | Không xác định — thăm dò          | `200 OK`{ "message": "Profile updated" } | Success mang tính chất thăm dò tương tự như việc chỉ update name và không update sđt       |
| TC-C10 | EC-P9 (Phone định dạng +84)          | Body: `{ name: "Test User", phone: "+84912345678" }`  | Không xác định — thăm dò          | `200 OK`{ "message": "Profile updated" } | Success nhưng có vẻ như trường phone: không được xử lý ở backend mà chỉ dùng để lấy giá trị |
...
| TC-D3 | EC-S2 (Address rỗng)               | Body: `{ name: "Test", phone: "0912345678", shipping_address: "" }`               | Không xác định — thăm dò               | `200 OK`{ "message": "Profile updated" } | Success - Backend không xử lý giá trị của address       |
| TC-D4 | EC-S3 (Address null)               | Body: `{ name: "Test", phone: "0912345678", shipping_address: null }`             | Không xác định — thăm dò               | `200 OK`{ "message": "Profile updated" } | Success - Cập nhật cả giá trị null vào shipping address |
...
| STT   | Lớp bao phủ              | Input                                              | Expected Output                        | Actual                                     | Status                            |
| ----- | ------------------------ | -------------------------------------------------- | -------------------------------------- | ------------------------------------------ | --------------------------------- |
| TC-E1 | (Cố gắng thay đổi email) | Body: `{ name: "Test", email: "hacker@evil.com" }` | `400` hoặc `200` nhưng email không đổi | `200 OK`{ "message": "Profile updated" } | Success - Email không đổi         |
| TC-E2 | (Cố gắng thay đổi role)  | Body: `{ name: "Test", role: "admin" }`            | `400` hoặc `200` nhưng role không đổi  | `200 OK`{ "message": "Profile updated" } | Failure vì thay đổi được cả role. |
...
| BV-N3 | LB+1 = 256 ký tự | `"A"*256` | `{ name: "A"*256, phone: "0912345678" }` | `400` — vượt quá 255      | `200 OK`{ "message": "Profile updated" } | Failure - Có vẻ backend không xử lý thông tin về độ dài name |
...
| BV-P3 | LB-1 = 9 chữ số  | `"091234567"` (8 số + 1)     | `{ name: "Test", phone: "091234567" }`    | `400` — dưới 10 số      | `200 OK`{ "message": "Profile updated" } | Failure vì có vẻ backend được xử lý là đáy đúng giá trị được gửi lên |
| BV-P4 | LB+1 = 12 chữ số | `"091234567890"` (11 số + 1) | `{ name: "Test", phone: "091234567890" }` | `400` — trên 11 số      | `200 OK`{ "message": "Profile updated" } | Failure vì có vẻ backend được xử lý là đáy đúng giá trị được gửi lên |
| BV-P5 | LB = 0 số        | `""`                         | `{ name: "Test", phone: "" }`             | `400` — phone bắt buộc  | `200 OK`{ "message": "Profile updated" } | Failure vì có vẻ backend được xử lý là đáy đúng giá trị được gửi lên |
...
| STT   | Điểm biên         | Giá trị                         | Input                                    | Expected Output             | Actual                                     | Status                         |
| ----- | ----------------- | ------------------------------- | ---------------------------------------- | --------------------------- | ------------------------------------------ | ------------------------------ |
| BV-P6 | Không có số 0 đầu | `"1912345678"` (bắt đầu bằng 1) | `{ name: "Test", phone: "1912345678" }`  | `400` — phải bắt đầu bằng 0 | `200 OK`{ "message": "Profile updated" } | ☐                              |
| BV-P7 | Ký tự đầu là `+`  | `"+9123456789"`                 | `{ name: "Test", phone: "+9123456789" }` | Không xác định — thăm dò    | `200 OK`{ "message": "Profile updated" } | Failure vì đây chỉ là thmaw dò |
...
| --- | ----------------------------- | --------------------- | ------------------------------- | ----------------------------- | ------------------------------------- |
| 1   | TC-A1 (Token hợp lệ)          | *(điền sau khi test)* | Cập nhật thành công             | *(sẽ điền sau khi có Actual)* | —                                     |
| 2   | TC-A2/A3 (Auth)               | *(điền sau khi test)* | 401 Unauthorized                | *(sẽ điền sau khi có Actual)* | —                                     |
| 3   | TC-B2/B3/B4 (Name validation) | *(điền sau khi test)* | 400 — name không hợp lệ         | *(sẽ điền sau khi có Actual)* | Kiểm tra GET /api/users/me sau update |
| 4   | TC-C3/C4 (Phone length)       | *(điền sau khi test)* | 400 — phone phải 10–11 số       | *(sẽ điền sau khi có Actual)* | Kiểm tra GET /api/users/me            |
| 5   | TC-C5 (Phone không có 0)      | *(điền sau khi test)* | 400 — phone phải bắt đầu bằng 0 | *(sẽ điền sau khi có Actual)* | Kiểm tra GET /api/users/me            |
| 6   | TC-E1 (Thay đổi email)        | *(điền sau khi test)* | Email không thay đổi            | *(sẽ điền sau khi có Actual)* | Gọi GET /api/users/me để xác nhận     |
| 7   | TC-E2 (Thay đổi role)         | *(điền sau khi test)* | Role không thay đổi             | *(sẽ điền sau khi có Actual)* | Gọi GET /api/users/me để xác nhận     |
...
| -------------------------------- | ------------ | ----- | ----- | ----------------------------- |
| A. Xác thực                      | 3            | —     | —     | —                             |
| B. Validation `name`             | 5            | —     | —     | Kiểm tra GET sau update       |
| C. Validation `phone`            | 10           | —     | —     | Kiểm tra GET sau update       |
| D. Validation `shipping_address` | 4            | —     | —     | Kiểm tra GET sau update       |
| E. Security (email/role)         | 2            | —     | —     | Gọi GET để xác nhận không đổi |
| BV (Boundary)                    | 7            | —     | —     | —                             |
| **Tổng cộng**                    | **31**       | **—** | **—** | —                             |
...
| ------------- | ------------------------------ | -------- | ------------------ |
| FR-04-BUG-001 | *(điền sau khi phát hiện bug)* | —        | *(đường dẫn file)* |
