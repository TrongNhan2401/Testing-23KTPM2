...
| STT   | Lớp bao phủ               | Input                                                                                                                     | Expected Output                                                                                                                                                                                                                           | Actual                                                                                                                                                                                                                                              | Status                                                                                           |
| ----- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| TC-B1 | EC-T1 (Sai total_amount)  | Body: `{ total_amount: 1, shipping_address: "123 Lê Lợi" }` — gửi giá trị 1 VNĐ trong khi cart thực tế có giá trị lớn hơn | `200` với **đơn hàng có final_amount = tổng thực từ giỏ hàng** (backend đã ignore giá trị client). Sau đó gọi `GET /api/orders/:id` để xác nhận `final_amount` trong response đơn hàng khớp với tổng cart, không phải giá trị client gửi. | `200 OK` `{ "message": "Checkout successful", "orderId": 2 }` — Sau khi xem đơn hàng: `200 OK` `{ "id": 2, "user_id": 2, "total_amount": 1, "status": "pending", "shipping_address": "123 Lê Lợi, Q.1, TP.HCM", "created_at": "2026-07-06 04:18:56" }` | ❌ **Failure** — Đơn hàng nhận `total_amount: 1` từ client thay vì tính lại từ giỏ hàng |
| TC-B2 | EC-T1 (Đúng total_amount) | Body: `{ total_amount: <giá trị bất kỳ>, shipping_address: "..." }` — gửi đúng giá trị cart để so sánh với TC-B1          | `200` — thành công. Kết quả `final_amount` trong đơn hàng phải giống TC-B1 (cùng tổng cart).                                                                                                                                              | *(điền sau khi test bằng Postman)*                                                                                                                                                                                                                  | ❌ **Failure** — Do đơn hàng lấy `total_amount` từ client nên giá trị phụ thuộc vào giá trị gửi lên |
| TC-B3 | EC-T2 (total_amount = 0)  | Body: `{ total_amount: 0, shipping_address: "..." }` — thăm dò đơn hàng miễn phí                                          | Không xác định trước — xem backend có tạo đơn hàng với `final_amount = 0` được không                                                                                                                                                      | `200 OK` `{ "message": "Checkout successful", "orderId": 4 }`                                                                                                                                                                                           | ✅ Success — Backend cho phép tạo đơn hàng `total_amount = 0` (nhưng do lấy từ client, không phải tính lại) |

### 3.3 Nhóm C: Giỏ hàng trống

| STT   | Lớp bao phủ              | Input                                   | Expected Output                                        | Actual                                                                                                                                                                                                             | Status                                                                                                                                                                |
| ----- | ------------------------ | --------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TC-C1 | EC-C2 (Cart trống)       | Đăng nhập → checkout khi giỏ hàng trống | `400`/`422` — không thể checkout khi không có sản phẩm | Sau khi chạy `/api/cart`: `200 OK` `[]` (giỏ hàng trống). Sau khi chạy `/api/checkout`: `200 OK` `{ "message": "Checkout successful", "orderId": 3 }`                                                          | ❌ **Failure** — Checkout thành công khi giỏ hàng trống, vi phạm nghiệp vụ cơ bản                                                                                  |
| TC-C2 | EC-C1 (Cart có sản phẩm) | Cart trống → thêm 1 sản phẩm → checkout | `200` — thành công                                     | `200 OK` `{ "message": "Checkout successful", "orderId": 5 }`. Tuy nhiên khi gọi `/api/cart` sau checkout: `200 OK` `{ "id": 1, "name": "Sản phẩm A", "price": 100000, "quantity": 2 }` — giỏ hàng **vẫn còn sản phẩm** | ❌ **Failure** — Đơn hàng tạo thành công nhưng giỏ hàng không bị xóa sau checkout (vi phạm spec FR-08: "Sau thanh toán thành công, giỏ hàng được xóa") |
...
| TC-D2 | EC-S2 (`""` rỗng)        | Body: `{ shipping_address: "" }`                                                                          | `400 Bad Request` — địa chỉ bắt buộc | `200 OK` `{ "message": "Checkout successful", "orderId": 7 }` | ❌ **Failure** — Backend không validate rỗng string   |
| TC-D3 | EC-S3 (`null`)           | Body: `{ shipping_address: null }`                                                                        | `400 Bad Request`                    | `200 OK` `{ "message": "Checkout successful", "orderId": 8 }` | ❌ **Failure** — Backend không validate null           |
| TC-D4 | EC-S4 (chỉ khoảng trắng) | Body: `{ shipping_address: " " }`                                                                         | `400` — coi như rỗng                 | `200 OK` `{ "message": "Checkout successful", "orderId": 9 }`  | ❌ **Failure** — Backend không validate khoảng trắng   |
| TC-D5 | EC-S3 (thiếu trường)     | Body: `{}` — không gửi `shipping_address`                                                                 | `400 Bad Request` — trường bắt buộc  | `200 OK` `{ "message": "Checkout successful", "orderId": 10 }` | ❌ **Failure** — Gửi body rỗng nhưng vẫn chấp nhận thanh toán |
...
| ----- | --------------------- | -------------------------------------------------- | ------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| BV-S1 | Min = 1 ký tự         | `"A"`                                              | `{ shipping_address: "A" }`           | `200` — hợp lệ về mặt độ dài                             | `200 OK` `{ "message": "Checkout successful", "orderId": 11 }` | ✅ Success — 1 ký tự được chấp nhận (confirm EC-S1: tối thiểu 1 ký tự)                                |
| BV-S2 | Probing: 500 ký tự    | 500 ký tự `"A"*500`                                | `{ shipping_address: "A"*500 }`       | Không xác định trước — thăm dò max length thực            | `200 OK` `{ "message": "Checkout successful", "orderId": 12 }` | ✅ Success — Backend không có max length validation cho `shipping_address`                              |
| BV-S3 | Probing: 1000 ký tự   | 1000 ký tự `"A"*1000`                              | `{ shipping_address: "A"*1000 }`      | Không xác định trước — thăm dò max length                 | `200 OK` `{ "message": "Checkout successful", "orderId": 14 }` | ✅ Success — Backend chấp nhận chuỗi 1000 ký tự, confirm không có max length                         |
| BV-S4 | Ký tự đặc biệt / HTML | `"<script>alert(1)</script>"` hoặc `"Le\tLoi\nQ1"` | `{ shipping_address: "<script>..." }` | Không xác định trước — thăm dò sanitization / validation | `200 OK` `{ "message": "Checkout successful", "orderId": 13 }` | ✅ Success — Backend không sanitize/validate HTML injection trong `shipping_address`                      |
...
| --- | -------------------------------------------------------------- | ------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| G1  | Độ dài tối thiểu / tối đa của `shipping_address`               | Spec FR-08 không nêu con số cụ thể                                             | **ĐÃ XÁC NHẬN:** Tối thiểu = 1 ký tự, không có max length (BV-S1..BV-S4) |
| G2  | HTTP status code khi giỏ hàng trống                            | Spec FR-08 / api_specification không nêu                                       | **ĐÃ XÁC NHẬN:** Backend trả `200` thay vì `400`/`422` → BUG         |
| G3  | Trường `total_amount` trong API body là bắt buộc hay optional? | `api_specification.md` §4.3 liệt kê trường này nhưng FR-08 nói backend tự tính | **ĐÃ XÁC NHẬN:** `total_amount` KHÔNG bắt buộc (body `{}` vẫn tạo đơn hàng) |
| G4  | Xử lý đơn hàng miễn phí (`total_amount = 0` sau khi giảm giá)  | Spec không nêu                                                                 | **ĐÃ XÁC NHẬN:** Backend cho phép tạo đơn hàng `final_amount = 0` (nhưng do lấy từ client) |

### 5.2 Ràng buộc giả định hợp lý (cần xác nhận với business/QA)

| #   | Giả định                                     | Lý do giả định                                                                                        | Kết quả test                              |
| --- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| GA1 | Checkout với giỏ hàng trống phải bị từ chối  | Nghiệp vụ: không thể thanh toán khi không có sản phẩm                                                 | ❌ **VI PHẠM** — TC-C1 checkout thành công |
| GA2 | `shipping_address` không được rỗng             | Nghiệp vụ: cần địa chỉ để giao hàng                                                                   | ❌ **VI PHẠM** — TC-D2/D3/D4/D5 đều pass |
| GA3 | Backend phải ignore `total_amount` từ client | FR-08: "Backend phải tự tính lại tổng tiền; không chấp nhận giá trị `total_amount` do client gửi lên" | ❌ **VI PHẠM NGHIÊM TRỌNG** — TC-B1      |
| GA4 | Giỏ hàng bị xóa sau checkout thành công      | FR-08: "Sau thanh toán thành công, giỏ hàng được xóa"                                                  | ❌ **VI PHẠM** — TC-C2 giỏ hàng vẫn còn   |
...
| --- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| 1   | TC-B1 (Nhóm B — Total amount) | Backend nhận `total_amount: 1` từ client và lưu y nguyên vào CSDL (`orderId: 2` có `total_amount: 1`). Quan sát thấy: gọi `GET /api/orders/:id` trả về đúng giá trị client gửi, không phải tổng từ giỏ hàng.                                                     | Backend ignore `total_amount` client → `final_amount` = tổng cart  | **Xác nhận bằng chứng:** Backend hoàn toàn không tính lại `total_amount` từ giỏ hàng. Cần kiểm tra xem có bất kỳ trường hợp nào backend tự tính không (ví dụ khi `total_amount` không gửi). | Xác nhận TC-B2 — khi gửi giá trị khác thì kết quả cũng phụ thuộc vào client |
| 2   | TC-C1 (Nhóm C — Cart trống)   | Gọi `/api/cart` → giỏ hàng trả về `[]` (mảng rỗng). Gọi `/api/checkout` → trả về `200 OK` với `orderId: 3`. Đơn hàng được tạo thành công dù giỏ hàng trống.                                                                                         | Cart trống → từ chối checkout (`400`/`422`)                       | Backend không kiểm tra trạng thái giỏ hàng trước khi checkout. Đây là bug nghiêm trọng vì tạo đơn hàng "ma". | Kiểm tra message lỗi — không có, chỉ có success message |
| 3   | TC-A2                         | Không có token → trả về `401 Unauthorized` với body `{ "error": "Unauthorized" }`. Đúng như kỳ vọng.                                                                                                                                                     | Không có token → `401`                                            | Không có giả thuyết — hành vi đúng spec.                                                             | —                                                                 |
| 4   | BV-S2 / BV-S3                 | Backend chấp nhận chuỗi 500 và 1000 ký tự `"A"` mà không báo lỗi. Không có max length validation.                                                                                                                                                        | Không xác định — thăm dò max length                              | Backend không enforce max length cho `shipping_address`. Có thể lưu chuỗi không giới hạn vào CSDL.    | Xác nhậh max length thực qua kiểm tra CSDL (nếu có quyền)      |
| 5   | TC-B3 (total_amount = 0)      | Backend cho phép tạo đơn hàng `orderId: 4` với `total_amount = 0`. Tuy nhiên giá trị này là do client gửi lên, không phải backend tính lại.                                                                                                          | Không xác định — thăm dò đơn hàng miễn phí                       | Đơn hàng miễn phí được tạo (do lấy từ client). Điều này có thể khai thác được: khách hàng gửi `total_amount: 0` để thanh toán miễn phí. | Kiểm tra xem order có trường nào cho biết đơn hàng này bị manipulation không |
...
| C. Giỏ hàng trống                     | 2            | 0    | 2    | Kiểm tra trạng thái giỏ hàng sau checkout (bug TC-C2)            |
| D. Shipping address                   | 5            | 1    | 4    | Tất cả validation đều bị bypass — cần fix backend                |
| BV-S (biên shipping address)          | 4            | 4    | 0    | — (chỉ thăm dò, không kỳ vọng validation ở boundary)             |
| **Tổng cộng**                         | **17**       | **9**| **8**| |

**Tỷ lệ pass: 52.9% (9/17) — 8 bug nghiêm trọng cần fix**
...
| FR-08-BUG-002   | Checkout thành công khi giỏ hàng trống — vi phạm nghiệp vụ cơ bản                             | Critical | `testing/FR-08/issues/FR-08-BUG-002.md` |
