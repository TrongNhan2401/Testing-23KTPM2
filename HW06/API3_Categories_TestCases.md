...
| TestID | Kỹ thuật (Technique) | Mô tả Test Case | Header & Request Body (JSON) | Status Code Mong Đợi | Assertions / Expected Response | Đánh giá Audit (Human Review) | Ghi chú Audit / Nhận xét phản biện của Sinh viên (MSSV: 23127443) |
| :--- | :--- | :--- | :--- | :---: | :--- | :---: | :--- |
| **TC01** | Domain / Valid | Tạo danh mục mới thành công với quyền Admin | `Auth: AdminToken`<br>`{"name": "Điện thoại"}` | `201 Created` / `200` | Trả về object danh mục chứa `id` mới và `name == "Điện thoại"`. | **VALID** | Giữ nguyên. Khớp FR-14. |
...
| **TC05** | Domain / Boundary | Tạo danh mục tên có độ dài 1 ký tự | `Auth: AdminToken`<br>`{"name": "A"}` | `201 Created` / `200` | Độ dài tối thiểu hợp lệ. | **VALID** | Giữ nguyên. Tên 1 ký tự hợp lệ. |
| **TC06** | Domain / Boundary | Tạo danh mục tên vừa đúng độ dài max (255 ký tự) | `Auth: AdminToken`<br>`{"name": "A...A"}` (255 chars) | `201 Created` / `200` | Độ dài tối đa hợp lệ. | **INVALID** | **AI Hallucination:** README FR-14 chỉ nói "bắt buộc, không để trống", không quy định giới hạn max 255 chars như Product. |
| **TC07** | Domain / Invalid | Tạo danh mục tên rỗng (Empty string) | `Auth: AdminToken`<br>`{"name": ""}` | `400 Bad Request` | Không cho phép tên danh mục rỗng. | **VALID** | Giữ nguyên. Khớp FR-14 "không được để trống". |
...
| **TC09** | Domain / Invalid | Tạo danh mục thiếu trường `name` trong body | `Auth: AdminToken`<br>`{}` | `400 Bad Request` | Lỗi thiếu trường bắt buộc `name`. | **VALID** | Giữ nguyên. Khớp FR-14. |
...
| **TC14** | Domain / Invalid | Tạo danh mục tên vượt quá 255 ký tự | `Auth: AdminToken`<br>`{"name": "A...A"}` (300 chars) | `400 Bad Request` | Lỗi độ dài tên quá giới hạn. | **INVALID** | **AI Hallucination:** FR-14 không khai báo giới hạn 255 ký tự cho tên danh mục. |
| **TC15** | Domain / Conflict | Tạo danh mục với tên đã tồn tại trong DB | `Auth: AdminToken`<br>`{"name": "Điện thoại"}` | `400` / `409 Conflict` | Từ chối trùng tên danh mục. | **INVALID** | **AI Giả định không căn cứ:** README/Spec không quy định tên Category phải duy nhất (khác Coupon Code ghi rõ unique). |
| **TC16** | Security / SEC-04 | **Leo thang quyền:** User thường cố tạo danh mục | `Auth: UserToken`<br>`{"name": "Hack Category"}` | `403 Forbidden` | **Bảo mật (SEC-04):** Từ chối User thường truy cập API Admin. | **VALID** | Giữ nguyên. Kiểm thử FR-12 / SEC-04 rất chuẩn. |
| **TC17** | Security / SEC-01 | Gửi Request không truyền Header `Authorization` | *(Không truyền Header)*<br>`{"name": "DM Test"}` | `401 Unauthorized` | Từ chối truy cập không có token. | **VALID** | Giữ nguyên. Khớp SEC-02. |
...
| **TC22** | Security / XSS | Stored XSS trong tên danh mục | `Auth: AdminToken`<br>`{"name": "<script>alert(1)</script>"}` | `201` / `400` | Sanitize HTML entities khi lưu và trả về UI. | **VALID** | Giữ nguyên. Khớp SEC-04. |
...
| **TC26** | Dataflow | Tạo danh mục mới và gọi `GET /api/categories` kiểm tra | `Auth: AdminToken`<br>`{"name": "Danh Mục Mới 2026"}` | `201 Created` | Danh mục mới tạo ngay lập tức xuất hiện trong `GET /api/categories`. | **VALID** | Giữ nguyên. Kiểm thử luồng dữ liệu hợp lệ. |
...
| **TC33** | Security | Cố tình truyền `role: "admin"` trong body bằng User Token | `Auth: UserToken`<br>`{"name": "DM", "role": "admin"}` | `403 Forbidden` | Không cho phép bypass role qua Body Payload (SEC-06). | **VALID** | Giữ nguyên. Khớp SEC-06. |
...
| **EXT02** | Xung đột đồng thời (Concurrency): 2 Admin bấm tạo cùng 1 tên danh mục trong 10ms | 2 Request `POST /api/categories` trùng tên gửi cùng lúc | 1 Request thành công (`201`), 1 Request thành công hoặc thất bại tùy thuộc DB Unique Index. | AI giả định DB có Unique Index mà không kiểm chứng cấu hình thực tế của bảng DB. |
| **EXT03** | Tạo danh mục có tên giống hệt nhưng khác hoa/thường (`Dien Thoai` vs `dien thoai`) | `{"name": "dien thoai"}` khi đã có `Dien Thoai` | Trả về `201` thành công (do Spec không quy định tên Category phải duy nhất). | AI tự suy đoán quy tắc Case-Insensitive Unique mà Spec không yêu cầu. |
